...
def scan_references(main_tex_path="main.tex"):
    ...
    try:
        ...
        logger.error(f"Error reading {main_tex_path}: {e}")
        return [], []
    
    # Find style and module references
    style_files = [f"style/{match}.sty" for match in re.findall(r'\\usepackage\{style/([^}]+)\}', content)]
    module_files = [f"modules/{match}.tex" for match in re.findall(r'\\input\{modules/([^}]+)\}', content)]
...
def test_basic_build(main_tex_path="main.tex"):
    ...
    try:
        ...
        if success:
            ...
            logger.error(f"LaTeX errors detected (check {temp_file}.log for details)")
...
def main():
    ...
    logger.info(f"Found {len(style_files)} style files and {len(module_files)} module files")
    ...
    if missing_files:
        ...
        for file_path in missing_files:
            logger.info(f"Creating template: {file_path}")
