...
run_pdflatex() {
    ...
    if pdflatex -output-directory="$BUILD_DIR" -interaction=nonstopmode "$MAIN_FILE" >> "$LOG_FILE" 2>&1; then
        log_message "${GREEN}✓ pdflatex pass $pass_num completed${NC}"
        analyze_log "$BUILD_DIR/$basename.log"
        return $?
    else
        log_message "${RED}✗ pdflatex pass $pass_num failed${NC}"
        analyze_log "$BUILD_DIR/$basename.log"
        return 1
    fi
}
...
run_bibtex() {
    ...
    if [[ -f "references.bib" ]] || grep -q "\\cite\|\\bibliography" "$MAIN_FILE" modules/*.tex 2>/dev/null; then
        ...
        if bibtex "$basename" >> "../$LOG_FILE" 2>&1; then
            cd ..
            log_message "${GREEN}✓ BibTeX completed${NC}"
            return 0
        else
            cd ..
            log_message "${YELLOW}⚠ BibTeX completed with warnings${NC}"
            return 0
        fi
    else
...
check_pdf_completeness() {
    ...
    if [[ ! -f "$pdf_file" ]]; then
        log_message "${RED}✗ PDF file not generated${NC}"
        return 1
    fi
    ...
    if [[ $pdf_size -lt 1024 ]]; then
        log_message "${RED}✗ PDF file too small (${pdf_size} bytes)${NC}"
        return 1
    fi
    ...
    if command -v pdfinfo &> /dev/null; then
        if pdfinfo "$pdf_file" &> /dev/null; then
            ...
            log_message "${GREEN}✓ PDF generated successfully with $pages pages (${pdf_size} bytes)${NC}"
        else
            log_message "${RED}✗ PDF appears to be corrupted${NC}"
            return 1
        fi
    else
        log_message "${GREEN}✓ PDF generated (${pdf_size} bytes)${NC}"
    fi
...
check_latex_guidelines() {
    ...
    if grep -r "\\usepackage.*{amsmath}" . --include="*.tex" &> /dev/null && 
       grep -r "\\usepackage.*{amssymb}" . --include="*.tex" &> /dev/null; then
        log_message "${GREEN}✓ AMS packages properly included${NC}"
    else
        log_message "${YELLOW}⚠ Consider including amsmath/amssymb packages${NC}"
        ((issues++))
    fi
    
    # Check for UTF-8 encoding
    if grep -r "\\usepackage.*{inputenc}" . --include="*.tex" &> /dev/null; then
        log_message "${GREEN}✓ Input encoding specified${NC}"
    else
        log_message "${YELLOW}⚠ Consider specifying input encoding${NC}"
        ((issues++))
    fi
    
    # Check for proper font encoding
    if grep -r "\\usepackage.*{fontenc}" . --include="*.tex" &> /dev/null; then
        log_message "${GREEN}✓ Font encoding specified${NC}"
    else
        log_message "${YELLOW}⚠ Consider specifying font encoding${NC}"
        ((issues++))
    fi
    
    # Check for language support
    if grep -r "\\usepackage.*{babel}" . --include="*.tex" &> /dev/null; then
        log_message "${GREEN}✓ Language support (babel) included${NC}"
    else
        log_message "${YELLOW}⚠ Consider including babel for language support${NC}"
        ((issues++))
    fi
