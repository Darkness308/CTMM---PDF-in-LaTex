...
run_latex_build() {
  ...
  log_message "${RED}[ERROR] LaTeX build failed${NC}"
  return 1
  fi
  else
  log_message "${RED}[ERROR] LaTeX build script not found${NC}"
  return 1
  fi
}
...
run_document_conversion() {
  ...
  log_message "${YELLOW}[WARN] Document conversion completed with issues${NC}"
  return 1
  fi
  else
  log_message "${RED}[ERROR] Document conversion script not found${NC}"
  return 1
  fi
}
...
run_error_analysis() {
  ...
  log_message "${YELLOW}[WARN] Error analysis completed (errors found)${NC}"
  return 1
  fi
  else
  log_message "${RED}[ERROR] Error analysis script not found${NC}"
  return 1
  fi
}
...
run_optimization() {
  ...
  log_message "${YELLOW}[WARN] Optimization analysis completed with recommendations${NC}"
  return 1
  fi
  else
  log_message "${RED}[ERROR] Optimization script not found${NC}"
  return 1
  fi
}
...
generate_master_report() {
  ...
  [[ -f "build/error-summary.txt" ]] && echo "[OK] build/error-summary.txt - Error summary"
  [[ -f "build/warning-summary.txt" ]] && echo "[OK] build/warning-summary.txt - Warning summary"
  [[ -f "build/conversion-log.txt" ]] && echo "[OK] build/conversion-log.txt - Conversion log"
  [[ -f "converted/integrated-content.tex" ]] && echo "[OK] converted/integrated-content.tex - Integrated converted content"
