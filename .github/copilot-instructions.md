...
- **Consequences of violation**: If this rule is violated, the LaTeX build will fail with the above error, and the PDF will not be generated until the offending `\usepackage` command is moved to the preamble. This can halt automated builds and block document updates.
