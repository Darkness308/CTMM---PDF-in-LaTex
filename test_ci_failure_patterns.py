...
def analyze_timeout_patterns() -> Dict[str, any]:
    ...
    print("⏱️  Analyzing Timeout Patterns")
    print("-" * 40)
    ...
    print(f"\n📊 Timeout Coverage: {timeout_analysis['steps_with_timeout']}/{timeout_analysis['total_steps']} ({coverage:.1f}%)")
...
def analyze_action_version_patterns() -> Dict[str, any]:
    ...
    print("\n📦 Analyzing Action Version Patterns")
    print("-" * 40)
    ...
    try:
        ...
        if result.returncode == 0:
            print("✅ Action version validation passed")
        else:
            print("❌ Action version validation failed")
            if result.stderr:
        ...
        print("⏱️  Action version validation timed out")
        ...
        print("❌ Action version validator not found")
...
def analyze_concurrency_patterns() -> Dict[str, any]:
    ...
    for workflow_file in workflow_files:
        ...
        try:
            ...
            if 'concurrency' in workflow_data:
                ...
                print(f"   ✅ {workflow_file}: Has concurrency control")
            else:
                print(f"   ⚠️  {workflow_file}: No concurrency control")
    ...
    print(f"\n📊 Concurrency Coverage: {concurrency_analysis['workflows_with_concurrency']}/{concurrency_analysis['total_workflows']} ({coverage:.1f}%)")
...
def run_validation_tests() -> Dict[str, any]:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print(f"   ✅ CTMM build system passed ({duration:.1f}s)")
        else:
            print(f"   ❌ CTMM build system failed ({duration:.1f}s)")
        ...
        print("   ⏱️  CTMM build system timed out")
        validation_results['tests_run'] += 1
    except Exception as e:
    ...
    try:
        ...
        if result.returncode == 0:
            validation_results['tests_passed'] += 1
            print(f"   ✅ Workflow syntax validation passed ({duration:.1f}s)")
        else:
            print(f"   ❌ Workflow syntax validation failed ({duration:.1f}s)")
        ...
        print(f"   ❌ Workflow syntax validation error: {e}")
        validation_results['tests_run'] += 1
...
def analyze_failure_patterns():
    ...
    for analysis_name, analysis_data in analyses.items():
        if 'issues' in analysis_data:
            ...
            if issue_count > 0:
                print(f"⚠️  {analysis_name.title()}: {issue_count} issues identified")
            else:
                print(f"✅ {analysis_name.title()}: No issues found")
    
    # Validation test summary
    validation = analyses['validation']
    if validation['tests_run'] > 0:
        success_rate = (validation['tests_passed'] / validation['tests_run']) * 100
        print(f"🧪 Validation Tests: {validation['tests_passed']}/{validation['tests_run']} passed ({success_rate:.1f}%)")
    
    # Overall health assessment
    print(f"\n🎯 Total Issues Identified: {total_issues}")
    
    if total_issues == 0:
        print("🎉 EXCELLENT: No failure patterns detected")
        return True
    elif total_issues <= 3:
        print("✅ GOOD: Minor issues identified, low failure risk")
        return True
    elif total_issues <= 6:
        print("⚠️  WARNING: Several issues identified, moderate failure risk")
        return True
    else:
        print("🔴 CRITICAL: Many issues identified, high failure risk")
        return False
