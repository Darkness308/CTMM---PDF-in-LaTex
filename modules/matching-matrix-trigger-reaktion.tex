...
\hline
\textbf{Vorwurf / Kritik} & Rückzug, Erstarren & Lautwerden, Rechtfertigung & [EMOJI] Ritualsatz, Ko-Regulation, Safe-Word \\
\hline
\textbf{Näheforderung ohne Warnung} & Reizüberflutung, Freeze & Impulsives Klammern & [EMOJI] Ankündigung, Rückzugsregel, Timer \\
\hline
\textbf{Kontrollverlust} & Frust, Bewegung, Fluchtimpuls & Angst, Überforderung & [RED] Strukturzettel, Rückkehrritual, Buddy-Call \\
\hline
\textbf{Unklarheit, was los ist} & Grübeln, Vermeidung & Drängen, wiederholen & [EMOJI] Matching-Check-In, CTMM-Visualisierung \\
\hline
\end{tabular}
\end{center}
...
\begin{itemize}
  \item \texttt{trigger\_forschungstagebuch} [SYM] individuelle Triggerdokumentation (\texttt{Kap. 5.2})
  \item \texttt{tool\_23\_triggermanagement} [SYM] Auslöser-Logik, Eskalationszyklus (\texttt{Kap. 5.3})
  \item \texttt{tool\_26\_co\_regulation} [SYM] Partnerintervention, Haltung in Echtzeit (\texttt{Kap. 2.6})
  \item \texttt{ritual\_workbook} [SYM] Rückkehrrituale nach Matching-Konflikt (\texttt{Kap. 3.4})
\end{itemize}

\begin{tcolorbox}[colback=ctmmPurple!5!white,colframe=ctmmPurple,title=Verwendung]
[EMOJI] \textbf{Diese Matching-Matrix ist besonders nützlich bei:}
\begin{itemize}
...
\textbf{Unser Matching wird:} \ctmmCheckBox{matching_besser}{Besser} \ctmmCheckBox{matching_gleich}{Gleich} \ctmmCheckBox{matching_hilfe}{Braucht Hilfe}
